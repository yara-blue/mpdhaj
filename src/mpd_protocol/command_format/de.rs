...
impl<'de> Deserializer<'de> {
    ...
    // Parse a string until the next whitespace character not within quotes
    // (quotes can be escaped with \")
    fn parse_string(&mut self) -> Result<String> {
        let mut output = String::new();
        let mut in_quotes = false;
        let mut char_indices = self.input.char_indices().chain([(self.input.len(), ' ')]);
        let (mut offset, first_char) = char_indices.next().expect("chained one element");
        let mut window = [' ', ' ', first_char];
        ...
        for (idx, char) in char_indices {
            ...
            match window {
                ['\\', '"', _] => output.push('"'),
                [_, '\\', '"'] => (),
                [_, '"', _] => in_quotes = !in_quotes,
                [_, ' ', _] if !in_quotes => break,
                [_, curr, _] => output.push(curr),
            }
        }
...
impl<'de, 'a> de::Deserializer<'de> for &'a mut Deserializer<'de> {
    ...
    where
        V: Visitor<'de>,
    {
        let ident = visitor.visit_borrowed_str(self.parse_ident()?)?;
